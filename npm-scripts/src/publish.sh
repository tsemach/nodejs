...
function usage() {
  ...
  echo "  -c | --compile: define the build target in package.json, like npm run build"
  echo ""
  echo "Examples"
  ...
  echo "  npx publish patch -a -t -c build:prod: with definition of build target"
...
while [ $# -gt 0 ]; do
  ...
  if [ X$arg = X"-c" -o X$arg = X"--compile" ]; then            
    withBuildTarget=$1
    shift
    continue
  fi
